...
**Total: 7,996 comprehensive tests**
...
|----------|-------|-------|-------|
| `unit` | 7,890 | 137 | Default primary category for files without explicit integration/e2e/smoke scope |
...
| **Primary Total** | **7,996** | **143** | **unit + integration + e2e + smoke** |

| CI Slice | Tests | Files | Selector |
|----------|-------|-------|----------|
| `test-unit` | 7,890 | 137 | `-m "unit and not slow"` |
...
| `test_watch_prevalidation.py` | 40 | Watch flag semantic prevalidation tests |
| `test_watch_command.py` | 6 | Watch command dispatch and signal handler tests |
| `test_watch_signals.py` | 2 | SIGINT/SIGTERM exit-0 contract tests |
| **Total** | **7,996** | **Collected via pytest --collect-only** |
...
- [x] Comprehensive test coverage (7,996 tests total)

...
class TestMainImplExitCodes:
    ...
    @pytest.mark.parametrize(
        "argv",
        [
            ["cja_auto_sdr", "--exit-codes", "--wrapper-job-id", "1"],
            ["cja_auto_sdr", "--exit-codes", "--agent-run-id", "abc123"],
            ["cja_auto_sdr", "--exit-codes", "--agent-run-id", "run_123"],
            ["cja_auto_sdr", "--exit-codes", "--agent-run-id", "r1"],
        ],
    )
    def test_unknown_wrapper_flag_is_ignored_for_raw_exit_codes_dispatch(self, argv, capsys):
    ...
    def test_flaglike_wrapper_boolean_is_ignored_for_raw_exit_codes_dispatch(self, capsys):
        with patch.object(sys, "argv", ["cja_auto_sdr", "--exit-codes", "--wrapper-debug", "false"]):
            with pytest.raises(SystemExit) as exc_info:
                _main_impl()
    ...
    def test_post_command_wrapper_value_that_looks_like_extra_arg_falls_through(self, capsys):
        with patch.object(sys, "argv", ["cja_auto_sdr", "--exit-codes", "--wrapper-job-id", "dv1"]):
            with pytest.raises(SystemExit) as exc_info:
                _main_impl()
    ...
    def test_post_command_wrapper_dataview_like_value_falls_through(self, capsys):
        with patch.object(sys, "argv", ["cja_auto_sdr", "--exit-codes", "--wrapper-job-id", "dv_12345"]):
            with pytest.raises(SystemExit) as exc_info:
                _main_impl()
    ...
    @pytest.mark.parametrize(
        "argv",
        [
            ["cja_auto_sdr", "--exit-codes", "--run-summary-json", "--wrapper-debug=bash", "stdout"],
            ["cja_auto_sdr", "--format", "--wrapper-debug=json", "--exit-codes"],
            ["cja_auto_sdr", "--exit-codes", "--agent-run-id"],
            ["cja_auto_sdr", "--sample-config", "--wrapper-job-id"],
        ],
    )
    def test_wrapper_interrupting_another_pending_option_falls_through(self, argv, capsys):
        with patch.object(sys, "argv", argv):
            with pytest.raises(SystemExit) as exc_info:
                _main_impl()
    ...
    def test_attached_wrapper_before_pending_run_summary_value_preserves_json_only_stdout(self, capsys):
        with patch.object(
            sys,
            "argv",
            ["cja_auto_sdr", "--exit-codes", "--run-summary-json", "--agent-run-id=abc123", "stdout"],
        ):
            with pytest.raises(SystemExit) as exc_info:
                main()
    ...
    def test_split_boolean_wrapper_before_pending_run_summary_value_preserves_json_only_stdout(self, capsys):
        with patch.object(
            sys,
            "argv",
            ["cja_auto_sdr", "--exit-codes", "--run-summary-json", "--agent-trace", "false", "stdout"],
        ):
            with pytest.raises(SystemExit) as exc_info:
                main()
    ...
    def test_split_identifier_wrapper_before_pending_run_summary_value_preserves_json_only_stdout(self, capsys):
        with patch.object(
            sys,
            "argv",
            ["cja_auto_sdr", "--exit-codes", "--run-summary-json", "--agent-run-id", "abc123", "stdout"],
...
class TestMainImplSampleConfig:
    ...
    @pytest.mark.parametrize(
        "argv",
        [
            ["cja_auto_sdr", "--sample-config", "--wrapper-job-id", "1"],
            ["cja_auto_sdr", "--sample-config", "--agent-run-id", "abc123"],
        ],
    )
    def test_unknown_wrapper_flag_is_ignored_for_sample_config(self, argv, capsys):
        with (
            patch("cja_auto_sdr.generator.generate_sample_config", return_value=True) as mock_gen,
    ...
    @patch("cja_auto_sdr.generator.generate_sample_config")
    def test_flaglike_wrapper_boolean_is_ignored_for_sample_config(self, mock_gen, capsys):
        mock_gen.return_value = True

        with patch.object(sys, "argv", ["cja_auto_sdr", "--sample-config", "--agent-trace", "false"]):
            with pytest.raises(SystemExit) as exc_info:
                _main_impl()
    ...
    @patch("cja_auto_sdr.generator.generate_sample_config")
    def test_sample_config_rejects_dataview_like_wrapper_value_after_command(self, mock_gen, capsys):
        with patch.object(sys, "argv", ["cja_auto_sdr", "--sample-config", "--wrapper-job-id", "dv_12345"]):
            with pytest.raises(SystemExit) as exc_info:
                _main_impl()
    ...
    @patch("cja_auto_sdr.generator.generate_sample_config")
    def test_sample_config_does_not_reinterpret_attached_wrapper_payload_as_command(self, mock_gen, capsys):
        with patch.object(sys, "argv", ["cja_auto_sdr", "--sample-config", "--wrapper-debug=--exit-codes"]):
            with pytest.raises(SystemExit) as exc_info:
                _main_impl()
...
class TestMainImplRunState:
    ...
    def test_raw_standalone_run_summary_syncs_split_wrapper_hidden_config_metadata(self, capsys):
        """Split wrapper IDs should not block config-file metadata recovery."""
        with patch.object(
            sys,
            "argv",
            [
        ...
        payload = json.loads(capsys.readouterr().out)
        assert payload["mode"] == "exit_codes"
        assert payload["config_file"] == "cfg.json"
    ...
    def test_invalid_raw_standalone_still_emits_stdout_summary_when_wrapper_hides_summary_target(self, capsys):
        """Unrelated standalone ambiguity must not suppress stdout run-summary output."""
        argv = [
            "cja_auto_sdr",
            "--completion",
            "--wrapper-job-id",
    ...
    def test_invalid_raw_standalone_still_honors_wrapper_hidden_output_dir(self, tmp_path, monkeypatch):
        """Recovered metadata should still route summary files under wrapper-hidden output directories."""
        monkeypatch.chdir(tmp_path)
        reports_dir = tmp_path / "reports"
        summary_path = reports_dir / "summary.json"

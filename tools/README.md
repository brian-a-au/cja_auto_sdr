...
- **`cja_sdr_generate`**: `data_view_id` is always required. `format` accepts the direct formats plus the CLI aliases `all`, `reports`, `data`, and `ci`. For single-SDR generation, `output` writes the single-file formats (`json`, `html`, `markdown`, `excel`) to the given file path, or streams JSON to stdout when set to `-`/`stdout` with `format: json`; multi-file formats (`csv`, `all`) and `notion` keep `output_dir` auto-naming. Standalone `quality_report` mode honors `output` for stdout or a caller-chosen file path, and falls back to `output_dir` for auto-named report files when `output` is omitted.

...
- **Snapshot parse cache:** refresh cached JSON after a file replacement even when its modification time and size are preserved, keeping diff discovery, retention metadata, and org-report history tied to the current file.
...
- **Uses the Notion 2025-09-03 data sources API.** The registry database is created and read through Notion's current data sources model (the property schema lives on the data source, and rows are parented to the data source). `--notion-database-id` takes the database ID and the tool resolves its data source automatically. The Notion client is pinned to `Notion-Version: 2025-09-03`, and the `[notion]` extra now requires `notion-client>=3.0.0`.
...
### Changed
- Added compatibility-preserving atomic write helpers (`write_json_atomic_compatible`, `write_text_atomic_compatible`) in `core/json_io.py` — new files get temp-file-and-rename durability while existing files preserve inode-backed overwrite semantics (symlink-following, hard-link visibility, xattr/ACL retention, file-mode preservation).
- Migrated 14 write sites across 10 writer modules (SDR, diff, org-report, inventory-summary, run-summary) to the new compatible atomic helpers.
- Converted 26 eager f-string logger calls to lazy `%s`-style interpolation across 8 writer modules.

### Fixed
- Output files are no longer silently truncated on write failure — existing files get fsync-and-flush durability; new files get atomic temp-and-rename.
...
### Refactor
- Extract diff text renderers and file writers into the `output.diff` package; `generator.py` delegates via thin wrappers
- Extract inventory summary display logic into the `output.inventory` package; `process_inventory_summary()` intentionally remains in `generator.py`
...
### Added
- **Structured diagnostic events** (`core/logging.py`): `emit_diagnostic()` helper and `ContextLoggerAdapter.emit_diagnostic()` with 7-event catalog — text mode formats as `[DIAG] event: k=v`, JSON mode merges into log records

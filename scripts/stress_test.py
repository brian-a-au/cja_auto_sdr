...
def test_validation_cache():
    ...
    for _i in range(10):
        ...
        checker_no_cache.check_all_quality_issues_optimized(df, "Metrics", ["id", "name", "type"], ["id", "name"])
    ...
    for _i in range(10):
        ...
        checker_with_cache.check_all_quality_issues_optimized(df, "Metrics", ["id", "name", "type"], ["id", "name"])
    ...
    if improvement >= 50:
        print("   ✓ EXCELLENT: Exceeds 50% improvement target")
    elif improvement >= 20:
        print("   ✓ GOOD: Meets 20% improvement threshold")
    else:
...
def test_parallel_validation():
    ...
    for _ in range(5):
        checker_seq.issues = []
        checker_seq.check_all_quality_issues_optimized(metrics_df, "Metrics", ["id", "name", "type"], ["id", "name"])
    ...
    for _ in range(5):
        ...
        checker_par.check_all_parallel(
            metrics_df, dimensions_df, ["id", "name", "type"], ["id", "name", "type"], ["id", "name"], max_workers=2
        )
    ...
    if improvement >= 10:
        print("   ✓ SUCCESS: Meets 10-15% improvement target")
    else:
...
def test_early_exit():
    ...
    df_invalid = pd.DataFrame({"wrong_field": [f"value_{i}" for i in range(1000)]})

    print("\n3a. Testing early exit with invalid data:")
    checker = DataQualityChecker(logger)
    start = time.time()
    checker.check_all_quality_issues_optimized(df_invalid, "Metrics", ["id", "name", "type"], ["id", "name"])
    ...
    if duration < 0.1:
        print("   ✓ EXCELLENT: Early exit working (< 0.1s)")
        return True
...
def test_performance_tracking():
    ...
    print(f"   ✓ Cache hits: {stats['hits']}")
    print(f"   ✓ Cache misses: {stats['misses']}")
    print("   ✓ Statistics tracked successfully")
...
def test_stress_scenarios():
    ...
    checker = DataQualityChecker(logger)
    start = time.time()
    checker.check_all_quality_issues_optimized(df_large, "Metrics", ["id", "name", "type"], ["id", "name"])
    duration = time.time() - start
    ...
    checker2.check_all_quality_issues_optimized(df_dupes, "Metrics", ["id", "name", "type"], ["id", "name"])
...
def test_concurrent_cache_access():
    ...
    cache = ValidationCache(max_size=100, ttl_seconds=3600, logger=logger)
    DataQualityChecker(logger, validation_cache=cache)
    ...
    def validate():
        try:
            ...
            c.check_all_quality_issues_optimized(df, "Metrics", ["id", "name", "type"], ["id", "name"])
    ...
    print("   ✓ 20 concurrent validations completed")
